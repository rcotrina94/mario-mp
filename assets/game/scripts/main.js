...
var Level = Base.extend({
	...
	reload: function() {
		...
		for(var i = this.figures.length; i--; ) {
			if(this.figures[i] instanceof Hero) {
...
var Star = ItemFigure.extend({
	...
	hit: function(opponent) {
		if(!this.taken && this.active && opponent instanceof Hero) {
			opponent.invincible();
			this.die();
		}
	},
...
var Mushroom = ItemFigure.extend({
	...
	hit: function(opponent) {
		if(this.active && opponent instanceof Hero) {
			if(this.mode === mushroom_mode.mushroom)
				opponent.grow();
...
var Bullet = Figure.extend({
	init: function(parent) {
		...
		this.parent = parent;
		this.setImage(images.mario_sprites, 191, 366);
		this.setSize(16, 16);
